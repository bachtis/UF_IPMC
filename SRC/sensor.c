...
/*======================================================================*/
int
mgmt_add( MGMT_CTRL_DEV_LOCATOR_RECORD *sdr )
{
	if( current_sensor_count + 1 > MAX_SENSOR_COUNT )
		return( -1 );

	sdr_entry_table[0].record_ptr = ( uchar * )sdr;
...
ipmi_get_sensor_reading( IPMI_PKT *pkt )
{
	...
	if (req->sensor_number == 1) {	//reserved for FRU Hot Swap sensor
		GET_SENSOR_READING_CMD_RESP *resp = ( GET_SENSOR_READING_CMD_RESP * )(pkt->resp);
		if( found ) {
			resp->completion_code = CC_NORMAL;
			resp->sensor_reading = sensor[i]->last_sensor_reading;
			resp->std_ipmi_byte = sensor[i]->std_ipmi_byte;
		...
		GET_SENSOR_READING_IPMB_CMD_RESP *resp = ( GET_SENSOR_READING_IPMB_CMD_RESP * )(pkt->resp);
		if( found ) {
			resp->completion_code = CC_NORMAL;
			//resp->sensor_reading = sensor[i]->last_sensor_reading;
			resp->std_ipmi_byte = sensor[i]->std_ipmi_byte;

			resp->ipmb_a_local_status = sensor[i]->ipmb_a_local_status;
			resp->ipmb_a_override_state = sensor[i]->ipmb_a_override_state;
			resp->ipmb_b_local_status = sensor[i]->ipmb_b_local_status;
			resp->ipmb_b_override_state = sensor[i]->ipmb_b_override_state;

			resp->ipmb_a_disabled_ipmb_b_disabled = sensor[i]->ipmb_a_disabled_ipmb_b_disabled;
			resp->ipmb_a_enabled_ipmb_b_disabled = sensor[i]->ipmb_a_enabled_ipmb_b_disabled;
			resp->ipmb_a_disabled_ipmb_b_enabled = sensor[i]->ipmb_a_disabled_ipmb_b_enabled;
			resp->ipmb_a_enabled_ipmb_b_enabled = sensor[i]->ipmb_a_enabled_ipmb_b_enabled;
...
the various SDRs from the FRUs IPM Controller to learn of the various objects
...
void
reserve_sdr_repository( IPMI_PKT *pkt )
{
  RESERVE_SDR_REPOSITORY_CMD_RESP *resp = ( RESERVE_SDR_REPOSITORY_CMD_RESP *)(pkt->resp);

  resp->reservation_id_lsb = 0;
  resp->reservation_id_msb = 0;
...
void
clear_sdr_repository( IPMI_PKT *pkt )
{
  CLEAR_SDR_REPOSITORY_CMD_RESP *resp = ( CLEAR_SDR_REPOSITORY_CMD_RESP *)(pkt->resp);
...
matches up with the retrieved record. If the Record Key information doesnt match,
...
the SDRs header and record key bytes.
*/
void
