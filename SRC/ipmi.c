	...
	switch( pkt->req->command )
	{
      ...
      gdi_resp->product_id[1] = 0x1B;
		...
		case IPMI_CMD_SET_WATCHDOG_TIMER:
			 ...
			 * command stops the timer (unless the dont stop bit is
...
the IPMB. The messages appear on the channel as if theyve come from BMC LUN 10b. Thus, if the
