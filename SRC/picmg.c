...
//#ifdef PICMG
	 ...
	 */
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_process_command: ingress\n" );
...
picmg_get_picmg_properties( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_picmg_properties: ingress\n" );

	resp->completion_code = CC_NORMAL;
	resp->picmg_id = PICMG_ID;
	resp->picmg_extension_ver_minor = 3;
 ...
 *
 ...
 a method for the host interface to retrieve information such as the devices
...
picmg_init( void )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_init: ingress\n" );
...
void
picmg_m0_state( unsigned fru_id )
{
	FRU_HOT_SWAP_EVENT_MSG_REQ msg;

    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_m0_state: ingress\n" );

	logger("Hot Swap Event Message", "M1 -> M0");

	/* set current state */
//	fru[fru_id].state = FRU_STATE_M0_NOT_INSTALLED;
	...
	msg.evt_data1 = 0xa << 4 | FRU_STATE_M0_NOT_INSTALLED;
...
picmg_m1_state( unsigned fru_id )
{
	FRU_HOT_SWAP_EVENT_MSG_REQ msg;
    	...
    	dbprintf( DBG_IPMI | DBG_INOUT, "picmg_m1_state: ingress\n" );
...
/*
   ...
   in the M5 state, the Payload functionality is not impacted. That is, from the Payloads perspective,
   M4 and M5 are equivalent.
   When the FRU receives the deactivation command, it moves on to the M6 state, and does whatever
   steps are necessary to shut down the FRUs Payload gracefully. The BLUE LED continues to blink
   ...
   Once the FRUs Payload is powered down, it transitions to the M1 state and sends the event to the
   Shelf Manager. The BLUE LED is turned on. When the Shelf Manager receives the M6 to M1
   transition event, it reclaims the FRUs power budget, removes the FRU from the SDR Repository,
...
{
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_handle_switch_state_change: ingress\n" );
...
/*
   ...
   may be read earlier in the FRUs life cycle (i.e., in either M2 or M3). When
   ...
   Port enables or disables. When the FRUs activation is complete, it sends an
   M3 to M4 event to inform the Shelf Manager that it is now active.
   Once a FRU has reached the M4 state, the Shelf Managers job becomes monitoring
...
picmg_set_fru_activation( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_set_fru_activation: ingress\n" );
	...
	if ( req->fru_dev_id < MAX_FRU_DEV_ID + 1 ) {
		switch( req->fru_activation ) {
			case FRU_CONTROL_DEACTIVATE_FRU:
				...
				} else if ( fru[req->fru_dev_id].state == FRU_STATE_M6_DEACTIVATION_IN_PROGRESS ) {
					picmg_m1_state( req->fru_dev_id );
				} else if ( fru[req->fru_dev_id].state == FRU_STATE_M3_ACTIVATION_IN_PROGRESS ) {
					picmg_m6_state( req->fru_dev_id );
				} else if ( fru[req->fru_dev_id].state == FRU_STATE_M2_ACTIVATION_REQUEST ) {
					picmg_m1_state( req->fru_dev_id );
				} else if ( fru[req->fru_dev_id].state == FRU_STATE_M4_ACTIVE ) {
			...
			case FRU_CONTROL_ACTIVATE_FRU:
				...
				} else if ( fru[req->fru_dev_id].state == FRU_STATE_M1_INACTIVE ) {
					picmg_m2_state( req->fru_dev_id );
...
picmg_set_power_level( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_set_power_level: ingress\n" );
...
/*
The Set FRU Activation Policy command modifies the way a FRUs operational state transitions
...
picmg_set_fru_activation_policy( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_set_fru_activation_policy: ingress\n" );
...
picmg_get_fru_activation_policy( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_fru_activation_policy: ingress\n" );
...
picmg_fru_control( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_fru_control: ingress\n" );
...
picmg_get_fru_led_properties( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_fru_led_properties: ingress\n" );
...
picmg_get_led_color_capabilities( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_led_color_capabilities: ingress\n" );
...
picmg_compute_power_properties( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_compute_power_properties: ingress\n" );
...
picmg_get_power_level( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_power_level: ingress\n" );
	...
	*/
	...
	// resp->power_multiplier = 0;


	/* Power Draw[1..N]. The first entry reflects the lowest level of power
	   (minimum power level) used by the FRUs Payload.  Power Draw[Max].
	   The last entry reflects the highest level of power used by the FRUs
	...
	// resp->power_draw[1] = 0xbe;

	toml_table_t* config;
	toml_table_t* pw_draw;
	toml_raw_t raw;
	long long int dsrd_std_state;
	...
	if (0 == (fp = fopen("/root/UF_IPMC/CONFIG/CONFIG.toml", "r"))) {
		logger("ERROR", "fopen() in picmg_get_power_level() (CONFIG.toml)");
	}

	/* Run the files through the parser. */
	config = toml_parse_file(fp, errbuf, sizeof(errbuf));
	if (0 == config) {
		logger("ERROR", "toml_parse_file() in picmg_get_power_level() (CONFIG.toml)");
	}

	fclose(fp);

	/* Locate the [POWER_DRAW] table. */
	if (0 == (pw_draw = toml_table_in(config, "POWER_DRAW"))) {
		logger("ERROR", "toml_table_in() 'POWER_DRAW' in picmg_get_power_level()");
	}

	/* Extract 'dsrd_std_state' config value. */
	if (0 == (raw = toml_raw_in(pw_draw, "desired_steady_state"))) {
		logger("ERROR", "toml_raw_in() 'desired_steady_state' in picmg_get_power_level()");
	}

	/* Convert the raw value into an int. */
	if (toml_rtoi(raw, &dsrd_std_state)) {
		logger("ERROR", "toml_rtoi() 'dsrd_std_state' in picmg_get_power_level()");
	}
	...
	switch (req->power_type) {
		case 0x00:
			resp->power_level = fru[req->fru_dev_id].power_level_steady_state;
			//resp->power_level = 1;
			//resp->dyn_power_config = fru[req->fru_dev_id].dyn_power_config;
			resp->dyn_power_config = 1;
			resp->delay_to_stable_power = fru[req->fru_dev_id].delay_to_stable_power;
			...
			break;
		case 0x01:
			resp->power_level = fru[req->fru_dev_id].power_level_desired_steady_state;
			//resp->power_level = 1;
			//resp->dyn_power_config = fru[req->fru_dev_id].dyn_power_config;
			resp->dyn_power_config = 1;
			resp->delay_to_stable_power = fru[req->fru_dev_id].delay_to_stable_power;
			...
			break;
		case 0x02:
			resp->power_level = fru[req->fru_dev_id].power_level_early_draw;
			//resp->power_level = 1;
			//resp->dyn_power_config = fru[req->fru_dev_id].dyn_power_config;
			resp->dyn_power_config = 1;
			resp->delay_to_stable_power = fru[req->fru_dev_id].delay_to_stable_power;
			...
			break;
		case 0x03:
			resp->power_level = fru[req->fru_dev_id].power_level_early_draw_desired;
			//resp->power_level = 1;
			//resp->dyn_power_config = fru[req->fru_dev_id].dyn_power_config;
			resp->dyn_power_config = 1;
			resp->delay_to_stable_power = fru[req->fru_dev_id].delay_to_stable_power;
...
{
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_shelf_address_info: ingress\n" );
...
{
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_set_shelf_address_info: ingress\n" );
	// Implement for the shelf manager
...
picmg_set_fru_led_state( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_set_fru_led_state: ingress\n" );
...
//			gpio_led_blink( led_mask, req->on_duration/10, req->led_function/10, 0 );
	...
	/* Color when illuminated. This byte sets the override color when
	   LED Function is 01hFAh and FFh. This byte sets the Local Control
...
picmg_get_fru_led_state( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_fru_led_state: ingress\n" );
...
picmg_get_device_locator_rec_id( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_device_locator_rec_id: ingress\n" );
...
picmg_set_port_state( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_set_port_state: ingress\n" );
...
picmg_get_port_state( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_port_state: ingress\n" );
...
{
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_renegotiate_power: ingress\n" );
	// Implement for the shelf manager
}
...
picmg_get_fan_speed_properties( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_fan_speed_properties: ingress\n" );
...
picmg_set_fan_level( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_set_fan_level: ingress\n" );
...
picmg_get_fan_level( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_fan_level: ingress\n" );
...
{
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_bused_resource: ingress\n" );
	// Implement for the shelf manager
}
...
picmg_get_ipmb_link_info( IPMI_PKT *pkt )
{
    ...
    dbprintf( DBG_IPMI | DBG_INOUT, "picmg_get_ipmb_link_info: ingress\n" );
